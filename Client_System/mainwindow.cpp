    ...
    , ui(new Ui::MainWindow)
{
    ...
    //发数据
    connect(my_house,&Myhouse::signal_sendCommondToClient,[=](QString key,QString value){
        if(socket->state() == QAbstractSocket::ConnectedState) //判断 客户端是否连接服务器 避免发送空数据 造成死机
            sendMessage(key,value);
    });

    //控制摄像头
    connect(my_camera,&Mycamera::signal_sendCommondToClient,[=](QString key,QString value){
        qDebug()<<"要发送给客户端的数据函数";
        if(socket->state() == QAbstractSocket::ConnectedState) //判断 客户端是否连接服务器 避免发送空数据 造成死机
            sendMessage(key,value);
    });
    //音乐播放器
    connect(my_music,&Mymusic::signal_sendCommondToClient,[=](QString key,QString value){
