    ...
    ui(new Ui::Mymusic)
{
    ...
    connect(ui->pushButton_add,&QPushButton::clicked,[=](){
        QList<QUrl> path = QFileDialog::getOpenFileUrls(this,"选择文件",QUrl("./"),"音频(*.mp3);;视频(*.mp4);;(*.*)");//获取文件路径
...
{
    QString name = ui->lineEdit->text();//获取输入的数据
    if(name.size() == 0 || name.size()>10)//对输入的数据合理性检查
    {
